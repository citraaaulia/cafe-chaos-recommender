# Di dalam file app.py - Updated Version

from flask import Flask, render_template, request, redirect, url_for, session, jsonify

# Import dari file/modul lain dalam proyek Anda
from models.database import init_mysql
from model.recommender_pakesoftganyambung import get_recommendations, update_feedback, get_recommendation_analytics, init_recommender, create_session
...
def create_app():
    ...
    def step7_budget():
        ...
        if request.method == 'POST':
            ...
            return redirect(url_for('step8_prioritas'))
    ...
    def step8_prioritas():
        ...
        # Pastikan semua data preferensi tersedia sebelum prioritas
        ...
        if request.method == 'POST':
            session['prioritas_1'] = request.form.get('prioritas_1')
            session['prioritas_2'] = request.form.get('prioritas_2')
            session['prioritas_3'] = request.form.get('prioritas_3')
            return redirect(url_for('hasil'))
        
        # Prepare question options for priority selection
        questions = {
            'mood': 'Suasana Hati',
            'rasa': 'Rasa Favorit',
            'tekstur': 'Tekstur Minuman',
            'kafein': 'Tingkat Kafein',
            'suhu': 'Suhu Minuman',
            'budget': 'Anggaran'
        }
        
        return render_template('step8_prioritas.html', 
                             questions=questions,
                             selected_prioritas={
                                 'prioritas_1': session.get('prioritas_1', ''),
                                 'prioritas_2': session.get('prioritas_2', ''),
                                 'prioritas_3': session.get('prioritas_3', '')
                             })
    ...
    def hasil():
        ...
        # Pastikan semua data preferensi tersedia
        required_fields = ['nama', 'mood', 'rasa', 'tekstur', 'kafein', 'suhu', 'budget', 'prioritas_1', 'prioritas_2', 'prioritas_3']
        for field in required_fields:
            if field not in session:
                return redirect(url_for('step1_nama'))
